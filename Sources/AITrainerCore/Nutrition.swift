...
public struct Nutrients: Codable, Equatable {
    ...
    public func validate() throws {
        guard [calories, protein, carbs, fat].allSatisfy({ $0.isFinite && $0 >= 0 }) else {
            throw TrainerError.invalid("Nutrient estimates must be finite and nonnegative.")
        }
    }
    public func scaled(by servings: Double) throws -> Nutrients {
        try validate()
        guard servings.isFinite, servings > 0 else { throw TrainerError.invalid("Servings must be greater than zero.") }
        let result = Nutrients(calories: calories * servings, protein: protein * servings, carbs: carbs * servings, fat: fat * servings)
