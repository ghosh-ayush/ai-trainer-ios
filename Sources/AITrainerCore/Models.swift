...
public struct SetLog: Codable, Equatable, Identifiable {
    ...
    public func validate() throws {
        guard reps >= 0, reps <= 1000, index >= 0, rir == nil || (0...10).contains(rir!) else {
            throw TrainerError.invalid("Use nonnegative reps and an optional RIR from 0 to 10.")
        }
        if let load, !load.isFinite || load < 0 { throw TrainerError.invalid("Load must be a nonnegative finite number.") }
    }
}
