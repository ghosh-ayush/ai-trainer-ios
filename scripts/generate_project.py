...
for path in sources:
    ...
    ref = add(
        "file:" + relative,
        f'isa = PBXFileReference; lastKnownFileType = sourcecode.swift; path = {q(relative)}; sourceTree = "<group>";',
    )
    builds.append(add("build:" + relative, f"isa = PBXBuildFile; fileRef = {ref};"))
    refs.append(ref)
privacy = add(
    "privacy",
    'isa = PBXFileReference; lastKnownFileType = text.xml; path = AITrainer/Resources/PrivacyInfo.xcprivacy; sourceTree = "<group>";',
)
resource = add("privacybuild", f"isa = PBXBuildFile; fileRef = {privacy};")
product = add(
    "product",
    "isa = PBXFileReference; explicitFileType = wrapper.application; path = AITrainer.app; sourceTree = BUILT_PRODUCTS_DIR;",
)
products = add(
    "products",
    f'isa = PBXGroup; children = ({product},); name = Products; sourceTree = "<group>";',
)
main = add(
    "main",
    f'isa = PBXGroup; children = ({",".join(refs + [privacy, products])},); sourceTree = "<group>";',
)
package = add("package", "isa = XCLocalSwiftPackageReference; relativePath = ../../..;")
package_product = add(
    "packageproduct",
    f"isa = XCSwiftPackageProductDependency; package = {package}; productName = AITrainerCore;",
)
framework_file = add("frameworkfile", f"isa = PBXBuildFile; productRef = {package_product};")
sourcephase = add(
    "sourcephase",
    f"isa = PBXSourcesBuildPhase; buildActionMask = 2147483647; files = ({','.join(builds)},); runOnlyForDeploymentPostprocessing = 0;",
)
resourcephase = add(
    "resourcephase",
    f"isa = PBXResourcesBuildPhase; buildActionMask = 2147483647; files = ({resource},); runOnlyForDeploymentPostprocessing = 0;",
)
frameworkphase = add(
    "frameworkphase",
    f"isa = PBXFrameworksBuildPhase; buildActionMask = 2147483647; files = ({framework_file},); runOnlyForDeploymentPostprocessing = 0;",
)
pythonphase = add(
    "pythonphase",
    'isa = PBXShellScriptBuildPhase; alwaysOutOfDate = 1; buildActionMask = 2147483647; files = (); inputPaths = (); outputPaths = (); name = "Bundle local Python"; runOnlyForDeploymentPostprocessing = 0; shellPath = /bin/sh; shellScript = '
    + q('/usr/bin/python3 "$PROJECT_DIR/../../../scripts/bundle_python.py"')
    + ";",
)
project_configs, target_configs = [], []
for configuration in ["Debug", "Release"]:
    project_settings = (
        """CLANG_ENABLE_MODULES = YES; SDKROOT = iphoneos; IPHONEOS_DEPLOYMENT_TARGET = 17.0; SWIFT_VERSION = 5.0;"""
    )
    ...
    if configuration == "Debug":
        settings += ' SWIFT_ACTIVE_COMPILATION_CONDITIONS = DEBUG; SWIFT_OPTIMIZATION_LEVEL = "-Onone"; DEBUG_INFORMATION_FORMAT = dwarf; ENABLE_TESTABILITY = YES;'
    else:
        settings += ' SWIFT_OPTIMIZATION_LEVEL = "-O"; DEBUG_INFORMATION_FORMAT = "dwarf-with-dsym";'
    project_configs.append(
        add(
            "project" + configuration,
            f"isa = XCBuildConfiguration; name = {configuration}; buildSettings = {{{project_settings}}};",
        )
    )
    target_configs.append(
        add(
            "target" + configuration,
            f"isa = XCBuildConfiguration; name = {configuration}; buildSettings = {{{settings}}};",
        )
    )
projectlist = add(
    "projectlist",
    f"isa = XCConfigurationList; buildConfigurations = ({','.join(project_configs)},); defaultConfigurationIsVisible = 0; defaultConfigurationName = Release;",
)
targetlist = add(
    "targetlist",
    f"isa = XCConfigurationList; buildConfigurations = ({','.join(target_configs)},); defaultConfigurationIsVisible = 0; defaultConfigurationName = Release;",
)
target = add(
    "target",
    f'isa = PBXNativeTarget; buildConfigurationList = {targetlist}; buildPhases = ({sourcephase},{frameworkphase},{resourcephase},{pythonphase},); buildRules = (); dependencies = (); name = AITrainer; packageProductDependencies = ({package_product},); productName = AITrainer; productReference = {product}; productType = "com.apple.product-type.application";',
)
project = add(
    "project",
    f'isa = PBXProject; attributes = {{LastUpgradeCheck = 1600; TargetAttributes = {{{target} = {{CreatedOnToolsVersion = 16.0; SystemCapabilities = {{com.apple.HealthKit = {{enabled = 1;}};}};}};}};}}; buildConfigurationList = {projectlist}; compatibilityVersion = "Xcode 14.0"; developmentRegion = en; hasScannedForEncodings = 0; knownRegions = (en,Base,); mainGroup = {main}; packageReferences = ({package},); productRefGroup = {products}; projectDirPath = ""; projectRoot = ""; targets = ({target},);',
)
