...
if not stdlib.is_dir(): raise SystemExit('Run scripts/setup_python.sh first; missing ' + str(stdlib))
# Purge stale modules in the build product only, never source or user data.
for name in ('python', 'app'):
    target = app / name
    if target.exists(): shutil.rmtree(target)
shutil.copytree(source.parent / 'lib/python3.13', app / 'python/lib/python3.13', ignore=shutil.ignore_patterns('__pycache__', '*.pyc', 'test', 'tests'))
...
shutil.copytree(ROOT / 'core/python/ai_trainer', app / 'app/ai_trainer', ignore=shutil.ignore_patterns('__pycache__', '*.pyc'))
...
for binary in (app / 'python').rglob('*.so'):
    ...
    if name not in {'math', '_json'}:
        ...
        continue
    framework = frameworks / (name + '.framework'); framework.mkdir(exist_ok=True)
    ...
    if privacy.exists(): shutil.copy2(privacy, framework / 'PrivacyInfo.xcprivacy')
    info = dict(CFBundleExecutable=name, CFBundleIdentifier='com.ghoshayush.AITrainer.python.' + name.replace('_','-'),
                CFBundleName=name, CFBundlePackageType='FMWK', CFBundleShortVersionString='3.13.11', CFBundleVersion='13',
                MinimumOSVersion='17.0', CFBundleSupportedPlatforms=['iPhoneSimulator' if simulator else 'iPhoneOS'])
