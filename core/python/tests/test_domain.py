...
from ai_trainer.contracts import validate, REQUEST
...
class DomainTests(unittest.TestCase):
    def setUp(self):
        ...
        self.p = self.request['payload']; self.state = self.p['state']; self.slot = self.state['program']['plans'][0]['slots'][0]
    def run_request(self, request=None):
        return json.loads(dispatch_json(json.dumps(request or self.request)))
    def reason(self): return self.run_request()['result']['reason']
    def test_golden_contract_and_no_input_mutation(self):
        ...
        self.assertEqual(self.run_request(), expected); self.assertEqual(self.request, before)
    ...
    def test_pain_overrides_qualifying_history(self):
        self.state['painExclusions'] = ['bench']; self.assertEqual(self.reason(), 'REPORTED_PAIN')
    def test_fixture_disabled_in_production(self):
        self.p['library']['permitsFixtures'] = False; self.assertEqual(self.reason(), 'POLICY_NOT_APPROVED')
    ...
    def test_modified_exposure_not_qualified(self):
        self.state['sessions'][-1]['plan']['modified'] = True; self.assertEqual(self.reason(), 'MODIFIED_EXPOSURE')
    def test_machine_loading_not_guessed(self):
        self.slot['equipment']['basis'] = 'machineSetting'; self.assertEqual(self.reason(), 'LOADING_POLICY_UNAVAILABLE')
    def test_missing_step_not_guessed(self):
        self.slot['equipment']['availableLoads'] = []; self.assertEqual(self.reason(), 'EQUIPMENT_STEP_UNKNOWN')
    def test_large_step_not_rounded_up(self):
        self.slot['equipment']['availableLoads'] = [100,110]; self.assertEqual(self.reason(), 'INCREMENT_EXCEEDS_BOUND')
    def test_deterministic_history_tie_break(self):
        for s in self.state['sessions']: s['startedAt'] = self.p['now'] - 86400
    ...
    def test_invalid_version_and_unknown_operation(self):
        self.request['schemaVersion'] = '2.0'; self.assertEqual(self.run_request()['error']['code'], 'unsupported')
        self.request['schemaVersion'] = '1.0'; self.request['operation'] = 'unknown'
        self.assertIn('error', self.run_request())
    def test_malformed_payloads_fail_closed(self):
        for mutate in [lambda: self.p['request'].update(minutes=True), lambda: self.slot.update(workingSets=0),
                       lambda: self.p.update(now=float('nan')), lambda: self.p['request'].update(slotID='bad-id')]:
            original = copy.deepcopy(self.request); mutate(); self.assertIn('error', self.run_request()); self.request=original; self.p=self.request['payload']; self.slot=self.p['state']['program']['plans'][0]['slots'][0]
    def recommendation(self, operation, state=None, **extra):
        p=dict(operation=operation,state=state or self.state,library=self.p['library'],now=self.p['now'],ids=[uid(800),uid(801)],**extra)
    ...
    def test_recommendation_is_proposal_until_accepted(self):
        result = self.proposal(); state=result['state']
    ...
    def test_changed_evidence_expires_acceptance(self):
        state=self.proposal()['state'];state['sessions'][-1]['logs'][0]['rir']=0
        self.assertEqual(self.recommendation('accept',state=state,id=uid(800),request=self.p['request'])['error']['code'],'staleProposal')
    def test_rejection_does_not_become_preference(self):
        ...
        self.assertEqual(result['profile'],self.state['profile']);self.assertEqual(result['recommendations'][0]['status'],'rejected')
    def test_nutrition_scaling_and_overflow(self):
        req=dict(schemaVersion='1.0',operation='nutrients',payload=dict(nutrients=dict(calories=200,protein=10,carbs=20,fat=5),servings=2))
        self.assertEqual(self.run_request(req)['result']['calories'],400)
        req['payload']['servings'] = 1e308;self.assertIn('error',self.run_request(req))
    def test_duplicate_catalog_rejected(self):
        e=dict(id='x',name='X',level='beginner',primaryMuscles=[],secondaryMuscles=[],instructions=[],category='strength',images=[])
    ...
    def test_recovery_is_unassessed_even_with_samples(self):
        ...
        result=self.run_request(req)['result'];self.assertEqual(result['status'],'unassessed');self.assertEqual(result['observations'],observations)
    def test_schema_copies_are_current(self):
        for name in ('request','response'):
            self.assertEqual((ROOT/f'shared/schemas/v1/{name}.schema.json').read_bytes(), (ROOT/f'core/python/ai_trainer/{name}.schema.json').read_bytes())
...
class StateReducerTests(unittest.TestCase):
    def setUp(self):
        ...
        self.state=self.base['state']; self.slot=self.state['program']['plans'][0]['slots'][0]
    def command(self, name, arguments, state=None):
        request=dict(schemaVersion='1.0',operation='stateCommand',payload=dict(command=name,state=state or self.state,
                     arguments=arguments,library=self.base['library'],now=self.base['now'],ids=[uid(900+i) for i in range(10)]))
        return json.loads(dispatch_json(json.dumps(request)))
    def test_configuration_is_pure_and_deduplicates_steps(self):
        result=self.command('configureLoad',dict(slotID=self.slot['id'],load=100,options=[105,100,105]))['result']['state']
    ...
    def test_failed_precondition_keeps_input_unchanged(self):
        ...
        result=self.command('start',dict(checkIn=dict(painReported=True,unavailableEquipment=[],occurredAt=self.base['now'])))
        self.assertIn('error',result);self.assertEqual(self.state,before)
    def test_stale_correction_preserves_both_values(self):
        session=self.state['sessions'][0];log=session['logs'][0]
        result=self.command('correctSet',dict(sessionID=session['id'],logID=log['id'],expectedRevision=0,load=105,reps=9))['result']
        self.assertFalse(result['value']);self.assertTrue(result['state']['sessions'][0]['logs'][0]['conflicted'])
        conflict=result['state']['conflicts'][0]
        self.assertEqual(conflict['current']['load'],100);self.assertEqual(conflict['incoming']['load'],105)
        self.assertNotIn('rir',conflict['incoming'])
    def test_meal_correction_is_audited(self):
        meal=dict(id=uid(600),revision=1,name='Meal',nutrients=dict(calories=100,protein=1,carbs=1,fat=1),occurredAt=self.base['now'],source='user_confirmed_estimate',timeZone='UTC')
    ...
    def test_delete_removes_related_evidence(self):
        ...
        self.assertEqual(len(result['sessions']),1);self.assertEqual(result['audits'],[])
    def test_missing_id_budget_rejected_before_reducer(self):
        request=dict(schemaVersion='1.0',operation='stateCommand',payload=dict(command='reportPain',state=self.state,arguments=dict(exerciseID='bench'),library=self.base['library'],now=self.base['now'],ids=[]))
